function imgMarkup(w, cls="") { if(!w.img) return `<div class="title-poster ${cls}"><span class="lang-zh">${w.title}</span><span class="lang-en">${w.en}</span><small>${w.year}</small></div>`; const src=w.img.startsWith("http")?w.img:w.img; return `<img class="${cls}" loading="lazy" src="${src}" alt="${w.title}">`; }
let data=[];
async function init(){
 data=window.WORKS||[];
...
function setLang(en){document.body.classList.toggle('english',en);document.documentElement.lang=en?'en':'zh-CN';document.querySelectorAll('.lang-zh').forEach(e=>e.style.display=en?'none':'');document.querySelectorAll('.lang-en').forEach(e=>e.style.display=en?'':'none');document.querySelectorAll('[data-lang]').forEach(e=>e.textContent=en?'中文':'EN');const si=document.querySelector('#search');if(si)si.placeholder=en?(si.dataset.placeholderEn||'Search works'):(si.dataset.placeholderZh||'搜索作品');document.title=(en?(document.body.dataset.titleEn||document.title):document.body.dataset.titleZh)||document.title;localStorage.setItem('linArchiveLang',en?'en':'zh')}
...
function mediaMarkup(w){
 ...
 return `<div class="media-gallery">${w.media.map(m=>`<figure><img loading="lazy" src="${m.url}" alt="${w.title}"><figcaption><span class="lang-zh">${m.caption||''}</span><span class="lang-en">${m.caption||''}</span></figcaption></figure>`).join('')}</div>`;
}
